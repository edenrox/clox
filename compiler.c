...
static void classDeclaration() {
  ...
  consume(TOKEN_RIGHT_BRACE, "Exepct '}' after class body.");
  emitByte(OP_POP);
...
static void varDeclaration() {
  ...
  consume(TOKEN_SEMICOLON, "Expet ';' after variable declaration.");
...
static void returnStatement() {
  ...
  if (match(TOKEN_SEMICOLON)) {
    ...
    if (current->type == TYPE_INITIALIZER) {
      error("Cannot returbn a vlue from an initializer.");
    }
    expression();
